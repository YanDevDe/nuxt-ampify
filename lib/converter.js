...
module.exports = async function (options, page, pageConfig, currentRoute) {
    ...
    if (options.localCache && cache[currentRoute]) {
        ...
        for (let i = as.length - 1; i >= 0; i--) {
            const a = as[i];
            let hrefLink = a.href.split(/(?:#|\?)/)[0]; //Remove # and ? from URL.
        ...
        let cssCharCounterOriginal = styleSingle.length;
...
async function processLocalCache(html, currentRoute, options) {
...
async function purgeUnusedCSS(html, style, options) {
    ...
    const stylePath = tempDir + `/${uniqueID}.css`;
    const htmlPath = tempDir + `/${uniqueID}.html`;
...
function classNameShortener(html, style) {
    ...
    // To avoid wrong replacement as example ".A" for ".replace" will not replaced at ".replace-that" to ".A-that"
...
async function processImageElement(html, localhostURL) {
    ...
    const result = await utils.replaceAsync(html, /<img([^>]*)>/gi, async (match, sub) => {
        //TODO: add this at personal config
        ...
        const widthAttributeExists = sub.indexOf("width") !== -1;
        const heightAttributeExists = sub.indexOf("height") !== -1;
        ...
        if (!widthAttributeExists || !heightAttributeExists) {
            ...
            //height exists, but width is missing
            else if (!widthAttributeExists && heightAttributeExists) {
            ...
            //height and width doesn't exists, so we use original size of image. 
            else if (!widthAttributeExists && !heightAttributeExists) {
